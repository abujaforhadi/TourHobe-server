...
export async function createPlan(hostId: string, data: any) {
  return prisma.travelPlan.create({
    data: {
      ...
      visibility: data.visibility ?? PUBLIC_VISIBILITY,
      hostId,
    },
  });
}
...
export async function matchPlans(query: {
  destination?: string;
  startDate?: string;
  endDate?: string;
  travelType?: string;
}) {
  const where: any = { visibility: PUBLIC_VISIBILITY };
...
export async function updatePlan(
  ...
  if (data.travelType !== undefined)
    updateData.travelType = data.travelType;
...
export async function requestToJoinPlan(planId: string, userId: string) {
  ...
  if (plan.hostId === userId) {
    throw AppError.badRequest(
      "You cannot request to join your own plan"
    );
  }
  ...
  if (existing) {
    throw AppError.badRequest(
      `You already have a request with status: ${existing.status}`
    );
  }
